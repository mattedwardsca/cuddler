...
{
    public static async Task<string?> FormatPageTitle(IHtmlHelper htmlHelper, ICuddlerModule? module, IApp? app)
    {
    ...
    public static string GetPagePath(IApp app, IMenuItem? page)
    {
        if (page == null)
        {
    ...
    public static IMenuItem? GetSelectedPage(HttpRequest request, IApp embeddedApp, bool preview, List<IMenuItem> menuLinks)
    {
    ...
    public static IMenuItem? GetSelectedPage(HttpContext context, IApp app, List<IMenuItem>? pageLinks)
    {
        if (pageLinks != null)
        {

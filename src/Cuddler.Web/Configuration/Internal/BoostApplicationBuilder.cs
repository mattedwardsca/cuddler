...
internal class BoostApplicationBuilder
{
    ...
    public BoostApplicationBuilder(WebApplicationBuilder applicationBuilder, Action<BoostConfiguration>? action = null)
    {
        action ??= _ => {
        };
    ...
    {
        // Web Services
        _applicationBuilder.Services.AddSingleton(_boostConfiguration.GlobalSettings);
        _applicationBuilder.Services.AddTransient<IClientAppService, ClientAppService>();

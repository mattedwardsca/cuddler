...
public class RazorPartialToStringRenderer : IRazorPartialToStringRenderer
{
    ...
    public async Task<string> RenderPartialToStringAsync<TModel>(string partialName, TModel model)
    {
        ...
        var viewDataDictionary = new ViewDataDictionary<TModel>(emptyModelMetadataProvider, modelStateDictionary)
        {
            Model = model
        };
        ...
        var viewContext = new ViewContext(actionContext, partial, viewDataDictionary, tempDataDictionary, output, htmlHelperOptions);
